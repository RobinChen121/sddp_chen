...
Date: 2025/2/25 21:34
Description: 
    Use the new feature of Gurobi--multiscenario
    in SDDP to solve the newsvendor problem.
-----
ini_I = 0
vari_cost = 1
unit_back_cost = 10
...
The multi scenario feature is designed for mixed-integer problems.
Thus, we always recommend to not apply it for purely continuous LPs.
...
inventory_constraints = [Any for _ in range(T)]
for t in range(T + 1):
    if t < T:
        q[t] = models[t].addVar(
    ...
    if t > 0:
        ...
        inventory_constraints[t - 1] = models[t].addConstr(I[t - 1] - B[t - 1] == 0)
        models[t].update()
...
while iter_ < iter_num:
    ...
    for t in range(0, T):
        ...
        models[t + 1].NumScenarios = scenario_forward_num
        ...
        for n in range(scenario_forward_num):  # parallel in this loop
            ...
            models[t + 1].params.ScenarioNumber = n
            inventory_constraints[t].ScenNRhs = rhs

        # optimize

        models[t + 1].optimize()
        models[t + 1].params.ScenarioNumber = 0
        s = models[t + 1].singleScenarioModel()
        pass


        for n in range(scenario_forward_num):
            models[t + 1].params.ScenarioNumber = n
            I_forward_values[t][n] = I[t].ScenNX
            B_forward_values[t][n] = B[t].ScenNX
            if t < T - 1:
                q_forward_values[t][n] = q[t+1].ScenNX
